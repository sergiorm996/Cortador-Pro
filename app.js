...
function eur(n, dec = 2) {
  if (isNaN(n)) n = 0;
  return Math.abs(n).toFixed(dec).replace('.', ',') + ' €';
}
...
function calcMes(mes, anio) {
  ...
  const cuota   = dias > 0 ? state.cfg.cuota : 0;
...
function recalc() {
  state.gastosDelDia = parseFloat(document.getElementById('gastos-dia').value) || 0;
        ...
        '<div class="hist-card">' +
          '<div class="hist-head">' +
            '<div>' +
              ...
              '<div class="hist-date">' + diaNom + ' ' + diaNum + ' &middot; ' + r.totalJamones + ' jamones &middot; ' + r.totalPlatos + ' platos</div>' +
            '</div>' +
...
function renderResumen() {
  ...
  document.getElementById('breakdown-meta').textContent = res.dias > 0 ? res.dias + ' jornadas' : '—';
  ...
  document.getElementById('md-irpf').textContent   = '−' + eur(res.irpf);
  document.getElementById('md-gastos').textContent = '−' + eur(res.gastos);
  document.getElementById('md-cuota').textContent  = '−' + eur(res.cuota);
}
...
/* ── AJUSTES / CONFIG ────────────────────────────────────────── */
function saveCfg() {
  state.cfg.iva   = parseFloat(document.getElementById('cfg-iva').value)   || 0;
  state.cfg.irpf  = parseFloat(document.getElementById('cfg-irpf').value)  || 0;
  state.cfg.cuota = parseFloat(document.getElementById('cfg-cuota').value) || 0;
  guardarEstado();
  recalc();
  renderResumen();
}
